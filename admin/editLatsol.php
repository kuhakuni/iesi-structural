...
if ($is_update === "put") {
	...
	$check = mysqli_query(
		$conn,
		"UPDATE latihan_soal SET judul = '$judul', id_matkul = '$matkul', link_file = '$link_file' WHERE id_latsol = $id "
	);
	if ($check) {
		echo "
        <script>
        alert('Latihan soal berhasil diubah!');
        window.location.href= './listLatsol.php';
        </script>
        ";
		exit();
	} else {
		echo "
        <script>
        alert('Latihan soal gagal diubah!');
        window.history.back();
        </script>
        ";
		exit();
...
$id = $_GET["id"];
$data = query("SELECT * FROM latihan_soal WHERE id_latsol = $id")[0];
if (is_null($data)) {
	header("Location: ./");
	exit();
}
...
<body>
    ...
    <div class="container">
        ...
        <div class="main">
            ...
            <div class="title">Edit Latihan Soal</div>
            <div class="content">
